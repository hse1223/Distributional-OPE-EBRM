Inaccuracy_Parameter_mix_Wasserstein <- function(theta_vec){
  ...
  for(state_index in 1:S_size){
    # state_index=1
    left_samples_candidate=rnorm(samplesize_candidate_sa[1, state_index], mean = muZ_left_candidate_vec[state_index], sd=sqrt(var_Z_true))    
    right_samples_candidate=rnorm(samplesize_candidate_sa[2, state_index], mean = muZ_right_candidate_vec[state_index], sd=sqrt(var_Z_true))    
